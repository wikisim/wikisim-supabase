...
setup_sentry("ef_update_data_component_v2")
...
import { EFDataComponentUpdateV2Returns } from "../_core/src/supabase/index.ts"
import {
...
Deno.serve(async req =>
{
    ...
    try
    {
        ...
        if (!auth_header)
        {
            return respond(401, { ef_error: "Missing or invalid Authorization header" })
        }
...
async function save_to_db(supabase: SupabaseClient, payload: EFUpdateDataComponentV2Args): Promise<Response>
{
    const { batch } = payload
    ...
    if (invalid_request_format || invalid_request_number_of_items)
    {
        const ef_error = invalid_request_format
            ? ERRORS.ERR16_update.message
            : ERRORS.ERR17_update.message
    ...
    if (!server_secret) return respond(500, { ef_error: ERRORS.ERR18_update.message })


    const response = await supabase.rpc("update_data_component_v2", { components, server_secret })

    const { error: rpc_error, data: rpc_data } = response
    if (rpc_error)
    {
        console.error("Error Updating data component:", rpc_error)
        let error_message = `${rpc_error.message}`
        if (!error_message.startsWith("ERR")) error_message = ERRORS.ERR19_update.message // Do not expose to public + error_message
    ...
    const rest_of_rpc_data: EFDataComponentUpdateV2Returns = rpc_data.map(data => {
